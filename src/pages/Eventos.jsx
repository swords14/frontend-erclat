...
// CORREÇÃO: Adicionado 'FileText' na importação abaixo
import { Plus, Search, Calendar, CheckCircle, XCircle, Tag, Clock, Pencil, Trash2, X as IconX, Check, Eye, Printer, DollarSign, FileText } from 'lucide-react';
...
export default function Eventos() {
    ...
    return (
        <div className="flex flex-col gap-6 p-4 md:p-6">
            ...
            {isLoading ? (
                <div className="text-center p-10">A carregar eventos...</div> 
            ) : eventosFiltrados.length === 0 ? (
                <div className="text-center p-16 bg-white dark:bg-gray-800 rounded-2xl">
                    {/* AQUI ESTAVA O ERRO: FileText agora está importado */}
                    <FileText size={48} className="mx-auto text-gray-400" />
                    <h3 className="mt-4 text-xl font-semibold">Nenhum evento encontrado</h3>
                </div>
            ) : (
...
function ModalEvento({ aberto, aoFechar, aoSalvar, evento, clientes, usuarios, inventoryItems, reservasEstoque, onFinalize }) {
    ...
    const handleSubmit = (e) => {
        ...
        // Conversões necessárias para o Backend
        const dataToSend = { 
            ...formData, 
        ...
        <AnimatePresence>
            {aberto && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-5xl flex flex-col max-h-[90vh]">
                        ...
                        <form onSubmit={handleSubmit} className="flex-grow overflow-hidden flex flex-col">
                            <div className="border-b dark:border-gray-700 flex-shrink-0 px-8"><nav className="flex space-x-8 overflow-x-auto"><TabButton tabId="geral">Geral</TabButton><TabButton tabId="local">Local & Logística</TabButton><TabButton tabId="detalhes">Detalhes</TabButton><TabButton tabId="equipe">Equipe & Tarefas</TabButton><TabButton tabId="estoque">Estoque</TabButton></nav></div>
                            <motion.div key={activeTab} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 space-y-6 overflow-y-auto flex-grow">
                                {activeTab === 'geral' && ( <div className="space-y-6"><h3 className="text-xl font-semibold">Informações Gerais</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4"><div><label className="label-form">Título*</label><input required name="title" value={formData.title} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Cliente*</label><select required name="clientId" value={formData.clientId} onChange={handleChange} className="input-form w-full"><option value="">Selecione...</option>{clientes.map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}</select></div><div><label className="label-form">Início*</label><input required type="datetime-local" name="startDate" value={formData.startDate} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Fim</label><input type="datetime-local" name="endDate" value={formData.endDate} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Convidados</label><input type="number" name="convidados" value={formData.convidados} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Valor (R$)</label><input type="number" step="0.01" name="valorTotal" value={formData.valorTotal} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Status</label><select name="status" value={formData.status} onChange={handleChange} className="input-form w-full"><option value="PLANEJADO">Planejado</option><option value="EM_ANDAMENTO">Em Andamento</option><option value="CONCLUIDO">Concluído</option><option value="FINALIZADO">Finalizado</option><option value="CANCELADO">Cancelado</option></select></div></div></div> )}
                                {activeTab === 'local' && ( <div className="space-y-6"><h3 className="text-xl font-semibold">Local & Logística</h3><div className="space-y-4"><div><label className="label-form">Nome do Local</label><input name="localNome" value={formData.localNome} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Endereço</label><input name="localEndereco" value={formData.localEndereco} onChange={handleChange} className="input-form w-full"/></div><div className="grid grid-cols-3 gap-4"><div><label className="label-form">Cidade</label><input name="localCidade" value={formData.localCidade} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Estado</label><input name="localEstado" value={formData.localEstado} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">CEP</label><input name="localCEP" value={formData.localCEP} onChange={handleChange} className="input-form w-full"/></div></div></div><div className="border-t pt-6 space-y-4"><h4 className="font-semibold">Montagem</h4><div className="grid grid-cols-3 gap-4"><div><label className="label-form">Data</label><input type="date" name="setupDate" value={formData.setupDate} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Início</label><input type="time" name="setupTimeStart" value={formData.setupTimeStart} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Fim</label><input type="time" name="setupTimeEnd" value={formData.setupTimeEnd} onChange={handleChange} className="input-form w-full"/></div></div></div><div className="border-t pt-6 space-y-4"><h4 className="font-semibold">Desmontagem</h4><div className="grid grid-cols-3 gap-4"><div><label className="label-form">Data</label><input type="date" name="teardownDate" value={formData.teardownDate} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Início</label><input type="time" name="teardownTimeStart" value={formData.teardownTimeStart} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Fim</label><input type="time" name="teardownTimeEnd" value={formData.teardownTimeEnd} onChange={handleChange} className="input-form w-full"/></div></div></div></div> )}
                                {activeTab === 'detalhes' && ( <div className="space-y-6"><h3 className="text-xl font-semibold">Detalhes Adicionais</h3><div className="grid grid-cols-2 gap-4"><div><label className="label-form">Tipo de Evento</label><input name="eventType" value={formData.eventType} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Tema</label><input name="eventTheme" value={formData.eventTheme} onChange={handleChange} className="input-form w-full"/></div></div><div><label className="label-form">Requisitos Específicos</label><textarea name="specificRequirements" value={formData.specificRequirements} onChange={handleChange} rows="3" className="input-form w-full"></textarea></div><div className="border-t pt-6 space-y-4"><h4 className="font-semibold">Contato no Local</h4><div className="grid grid-cols-2 gap-4"><div><label className="label-form">Nome</label><input name="eventContactName" value={formData.eventContactName} onChange={handleChange} className="input-form w-full"/></div><div><label className="label-form">Telefone</label><input type="tel" name="eventContactPhone" value={formData.eventContactPhone} onChange={handleChange} className="input-form w-full"/></div><div className="col-span-2"><label className="label-form">Email</label><input type="email" name="eventContactEmail" value={formData.eventContactEmail} onChange={handleChange} className="input-form w-full"/></div></div></div></div> )}
...
// Este componente agora está FORA da função ModalEvento
